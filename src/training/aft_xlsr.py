"""
Script for fine-tuning the XLS-R 1B model with adapter layers for ASR.

The XLS-R model (facebook/wav2vec2-xls-r-1b) uses the same Wav2Vec2ForCTC
architecture as MMS, so we can inject attention adapter layers via
...
# Bottleneck dimension for attention adapter layers (hidden_size -> dim -> hidden_size).
# XLS-R 1B has hidden_size=1280; 256 matches the MMS adapter design.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune XLS-R 1B model with adapter layers for ASR"
    )
    parser.add_argument(
        "lang",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory for model and tokenizer (default: models/xlsr/{lang})",
    )
    parser.add_argument(
        "--num-epochs",
        type=int,
...
def make_vocab(
    ...
    Unlike MMS, XLS-R uses a flat vocabulary dict (no language-key wrapper).
...
class DataCollatorCTCWithPadding:
    """
    Data collator that dynamically pads inputs and labels separately,
    since they have different lengths and require different padding methods.
...
def _model_card_language_metadata(lang: str) -> str:
    ...
    HF requires 'language' to be lowercase ISO 639-1/2/3; BCP-47 codes go in
    language_bcp47 (as array).
    """
...
# XLS-R 1B Adapter Fine-tuned for {LANGUAGES.get(lang, lang)}
...
- Fine-tuning method: Attention adapter layers (adapter_attn_dim={ADAPTER_ATTN_DIM})
...
"""
    ...
    adapter_file = ADAPTER_SAFE_FILE.format(lang)
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else config.models_dir / "xlsr" / lang / args.split
    training_logs_dir = config.results_dir / "training_logs"
    training_logs_dir.mkdir(parents=True, exist_ok=True)
    training_log_path = training_logs_dir / f"xlsr_aft_{args.split}_{lang}_{run_start_time:%Y%m%d_%H%M%S}.log"

    print("=" * 60)
    print(f"XLS-R 1B Adapter Fine-tuning for {lang} ({LANGUAGES[lang]}) split={args.split}")
    print("=" * 60)
    ...
    print("\nBuilding vocabulary...")
    vocab_path = make_vocab(train, val, output_dir)
    ...
    ############################################################################
    # Load pretrained XLS-R model with adapter layers injected via config.
    #
    # Passing adapter_attn_dim causes Wav2Vec2EncoderLayerStableLayerNorm to
    # create a Wav2Vec2AttnAdapterLayer in every encoder layer.  The adapter
    # weights are randomly initialised (not present in the XLS-R checkpoint).
    # init_adapter_layers() then re-initialises both adapters and the CTC head.
    print(f"\nLoading XLS-R model ({BASE_MODEL})...")
    model = Wav2Vec2ForCTC.from_pretrained(
        BASE_MODEL,
        adapter_attn_dim=ADAPTER_ATTN_DIM,
    ...
    if test_dataset is not None and len(test_dataset) > 0:
        ...
        if args.save_transcriptions:
            ...
            trans_path = trans_dir / f"transcriptions_xlsr_{lang}_{args.split}_{run_ts}.tsv"
    ...
    log_lines = [
        f"# XLS-R 1B Adapter Fine-tuning Run Log",
        f"run_start={run_start_time.isoformat()}",
        f"run_end={run_end_time.isoformat()}",
        f"model={BASE_MODEL}",
        f"adapter_attn_dim={ADAPTER_ATTN_DIM}",
        f"language={lang}",
        f"split={args.split}",
